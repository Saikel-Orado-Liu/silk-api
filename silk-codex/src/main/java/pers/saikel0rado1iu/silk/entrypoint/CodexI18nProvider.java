...
final class CodexI18nProvider extends I18nModInfoProvider {
	static final CodexI18nProvider EN_US = new CodexI18nProvider("en_us",
			"Silk API: Codex",
	...
	static final CodexI18nProvider ZH_CN = new CodexI18nProvider("zh_cn",
			"丝绸开发库：法典",
			"用于创建模组配置的子开发库。",
			"此系统包含了丝绸开发库中用于创建、读取、写入模组配置与切换配置模式的相关工具。", new String[]{
			"支持丝绸开发库"});
	static final CodexI18nProvider ZH_HK = new CodexI18nProvider("zh_hk",
			"絲綢開發庫：法典",
			"用於創建模組配置嘅子開發庫。",
			"此系統包含了絲綢開發庫中用於創建、讀取、寫入模組配置與切換配置模式嘅相關工具。", new String[]{
			"支持絲綢開發庫"});
	static final CodexI18nProvider ZH_TW = new CodexI18nProvider("zh_tw",
			"絲綢開發庫：法典",
			"用於建立模組配置的子開發庫。",
	...
	private CodexI18nProvider(String langCode, String name, String summary, String description, String[] allText) {

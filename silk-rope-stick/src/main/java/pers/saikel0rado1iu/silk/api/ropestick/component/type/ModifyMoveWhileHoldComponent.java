...
public record ModifyMoveWhileHoldComponent(ModifyMoveData modifyMove, boolean canModify) {
    /** 在持有时修改移动组件的默认值 */
    public static final ModifyMoveWhileHoldComponent DEFAULT =
            new ModifyMoveWhileHoldComponent(ModifyMoveData.DEFAULT, true);
    /** 在持有时修改移动组件的编解码器 */
    public static final Codec<ModifyMoveWhileHoldComponent> CODEC = RecordCodecBuilder
            .create(builder -> builder
                    .group(ModifyMoveData.CODEC
                                    .optionalFieldOf("modify_move", ModifyMoveData.DEFAULT)
                                    .forGetter(ModifyMoveWhileHoldComponent::modifyMove),
                            Codec.BOOL.optionalFieldOf("can_modify", true)
                                      .forGetter(ModifyMoveWhileHoldComponent::canModify))
                    .apply(builder, ModifyMoveWhileHoldComponent::new));
    /** 在持有时修改移动组件的数据包编解码器 */
    public static final PacketCodec<RegistryByteBuf, ModifyMoveWhileHoldComponent> PACKET_CODEC =

...
public abstract class BoltActionRepeatingFirearmItem extends CrossbowLikeItem {
	...
	public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
		...
		charged = false;
		loaded = false;
		maxUseTicks = RangedWeaponComponent.getQuickTicks(Math.round((float) rangedWeapon.maxUseTicks() * loadableAmount), stack);
		user.setCurrentHand(hand);
		return TypedActionResult.consume(stack);
	}
	...
	public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
		...
		if (!isCharged(stack) && maxUseTicks == RangedWeaponComponent.getQuickTicks(Math.round((float) rangedWeapon.maxUseTicks() * loadableAmount), stack)) {

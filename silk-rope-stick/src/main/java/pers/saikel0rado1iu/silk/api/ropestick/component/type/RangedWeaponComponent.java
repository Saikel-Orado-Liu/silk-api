...
public record RangedWeaponComponent(float maxSpeed,
    ...
    public static final RangedWeaponComponent CROSSBOW = builder()
            ...
            .build();
    /** 远程武器组件的编解码器 */
    public static final Codec<RangedWeaponComponent> CODEC = RecordCodecBuilder
            .create(builder -> builder
                    .group(Codec.FLOAT.fieldOf("max_speed")
                                      .forGetter(RangedWeaponComponent::maxSpeed),
                            Codec.FLOAT.fieldOf("max_damage")
                                       .forGetter(RangedWeaponComponent::maxDamage),
                            Codec.INT.fieldOf("max_use_ticks")
                                     .forGetter(RangedWeaponComponent::maxUseTicks),
                            Codec.INT.fieldOf("max_pull_ticks")
                                     .forGetter(RangedWeaponComponent::maxPullTicks),
                            Codec.FLOAT.fieldOf("firing_error")
                                       .forGetter(RangedWeaponComponent::firingError),
                            ItemStack.CODEC.fieldOf("default_projectile")
                                           .forGetter(RangedWeaponComponent::defaultProjectile),
                            Registries.ITEM.getCodec()
                                           .listOf()
                                           .fieldOf("launchable_projectiles")
                                           .forGetter(RangedWeaponComponent::launchableProjectiles))
                    .apply(builder, RangedWeaponComponent::new));
    /** 远程武器组件的数据包编解码器 */
    public static final PacketCodec<RegistryByteBuf, RangedWeaponComponent> PACKET_CODEC =
     ...
     * 获取发射物索引以供 JSON 渲染使用
     *
     * @param entity 拿着发射物品的实体
     * @param stack  发射物品堆栈
     * @return 索引
     */
    public float getProjectileIndex(LivingEntity entity, ItemStack stack) {
        if (entity.getActiveItem() == stack) {
            return getProjectileIndex(RangedWeaponComponent.getProjectileType(entity, stack));
        }
    ...
    public float getMaxProjectileSpeed(ItemStack stack) {
        ChargedProjectilesComponent component = stack
                .getOrDefault(CHARGED_PROJECTILES, ChargedProjectilesComponent.DEFAULT);
    ...
    public static class Builder {
        ...
        public RangedWeaponComponent build() {
            return new RangedWeaponComponent(maxSpeed, maxDamage, maxUseTicks, maxPullTicks,

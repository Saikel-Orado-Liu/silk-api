...
/**
 * <h2>生成实用工具</h2>
 * 此方法主要用于类似怪物类生物的生成，而不是动物类生物的生成。
 * <p>
 * 如果要实现动物类的生成要在 {@link Builder#otherChecker(SpawnRestriction.SpawnPredicate)} 中手动添加检测器
 * <p>
 * ！注意！在大部分情况下，此方法可能看起来失效。这是因为在
 * {@link SpawnSettings.Builder#spawn(SpawnGroup, SpawnSettings.SpawnEntry)} 中的设置的生成组规则大过自定义规则。此时将
 * {@link MobEntity#canSpawn(WorldView)} 与 {@link MobEntity#canSpawn(WorldAccess, SpawnReason)} 返回
 * true 则可以修复大部分怪物生成问题，生物会照样在区块生成时生成。虽无法预测此操作可能带来的完整效果，但经过测试，绝大部分情况下并不会破坏生成规则
 *
...
public interface SpawnUtil {
    ...
    final class Builder<T extends MobEntity> {
        ...
        private SpawnRestriction.SpawnPredicate<T> otherChecker = (t, w, s, p, r) -> true;
        ...
        public SpawnRestriction.SpawnPredicate<T> build() {
            return (type, world, spawnReason, pos, random) -> {
                try (ServerWorld serverWorld = world.toServerWorld()) {
                    return MobEntity.canMobSpawn(type, world, spawnReason, pos, random)
                            ...
                            && (!isDay || serverWorld.isDay())
                            && (!isNight || serverWorld.isNight())
